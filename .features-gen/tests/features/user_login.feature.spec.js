...
test.describe('Inicio de sesión de usuario en el sitio https://www.saucedemo.com/', () => {
  ...
  test('Añadir un producto al carrito de compras', { tag: ['@adicionarProductoAlCarrito'] }, async ({ Given, userLoginPage, When, userHomePageInventory, Then, userHomePageCart, page, userHomePageCheckout, And, userHomePageSummaryShopping, userHomePageFinishShopping }) => { 
    await Given('que el usuario ha iniciado sesión correctamente', null, { userLoginPage }); 
    ...
    await Then('no se visualiza el producto en el carrito de compras', null, { userHomePageCart, userHomePageCheckout }); 
    await And('se actualiza el valor de la compra', null, { userHomePageSummaryShopping, userHomePageCart }); 
    await When('cuando hace clic en el botón fihish', null, { userHomePageSummaryShopping }); 
    await Then('deberia visualizar el mensaje de compra exitosa', null, { userHomePageFinishShopping }); 
  });
